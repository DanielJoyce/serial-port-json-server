...
func (h *hub) run() {
	for {
		...
		case c := <-h.register:
			...
			c.send <- []byte("{\"Version\" : \"" + cfg.Version + "\"} ")
			...
			c.send <- []byte("{\"Hostname\" : \"" + *cfg.Hostname + "\"} ")
		case c := <-h.unregister:
...
func bufflowdebug(sl string) {
	...
	if strings.HasPrefix(sl, "bufflowdebug on") {
		*cfg.BufFlowDebugType = "on"
	} else if strings.HasPrefix(sl, "bufflowdebug off") {
		*cfg.BufFlowDebugType = "off"
	}
	h.broadcastSys <- []byte("{\"BufFlowDebug\" : \"" + *cfg.BufFlowDebugType + "\"}")
...
func restart() {
	...
	*/
	...
	cmd = exec.Command(exePath, "-ls", "-addr", *addr, "-regex", *regExpFilter, "-gc", *cfg.GCType)
